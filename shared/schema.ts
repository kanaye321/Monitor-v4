...
export const vmInventory = pgTable("vm_inventory", {
  ...
  // Host Information
  hypervisor: text("hypervisor").notNull(),
  ...
  vmStatus: text("vm_status").notNull().default('stopped'),
  vmIp: text("vm_ip"),
  // Internet Access
...
export const insertITEquipmentSchema = createInsertSchema(itEquipment).omit({ id: true });
...
export const iamAccounts = pgTable("iam_accounts", {
  ...
  permission: text("permission").notNull(), // Permission/IAM/SCOP
  ...
  remarks: text("remarks"),
  status: text("status").notNull().default("active"), // active, expired, extended, removed
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow()
});

export const insertIamAccountSchema = createInsertSchema(iamAccounts).omit({ id: true });
