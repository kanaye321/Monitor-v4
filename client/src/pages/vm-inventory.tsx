...
export default function VMInventoryPage() {
  ...
  const endIndex = Math.min(startIndex + itemsPerPage, filteredVMs.length);
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card>
              <CardHeader className="pb-1">
                ...
                <CardTitle className="text-3xl">{vms.filter(vm => (vm.vmStatus || vm.powerState || '').toLowerCase() === "running").length}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-1">
                <CardDescription>Stopped</CardDescription>
                <CardTitle className="text-3xl">{vms.filter(vm => (vm.vmStatus || vm.powerState || '').toLowerCase() === "stopped").length}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-1">
                <CardDescription>Internet Access</CardDescription>
                <CardTitle className="text-3xl">{vms.filter(vm => vm.internetAccess === true).length}</CardTitle>
              </CardHeader>
            </Card>
          </div>
        </CardContent>
      ...
      <Card>
            ...
            <Table>
              <TableHeader className="bg-muted/50">
                <TableRow>
				  ...
				  <TableHead>Host OS</TableHead>
				  <TableHead>VM OS</TableHead>
              ...
              <TableBody>
                {paginatedVMs.map((vm) => (
                  <TableRow key={vm.id}>
					...
					<TableCell>{vm.hostOs || 'Not set'}</TableCell>
					<TableCell>{vm.vmOs || 'Not set'}</TableCell>
					<TableCell>{vm.rack || 'Not set'}</TableCell>
        ...
        {/* Pagination */}
        {filteredVMs.length > itemsPerPage && (
            ...
            <div className="text-sm text-muted-foreground">
              <a 
                ...
                onClick={(e) => {
                  ...
                  const a = document.createElement('a');
                  a.href = url;
                  a.download = "vm_import_template.csv";
                  a.click();
                  URL.revokeObjectURL(url);
                }}
              >
